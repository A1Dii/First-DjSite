...
class MergeError(ValueError):
    ...
    Traceback (most recent call last):
    MergeError: Merge keys are not unique in left dataset; not a DjangoSite-to-DjangoSite merge
    """
...
class AbstractMethodError(NotImplementedError):
    ...
    def __init__(self, class_instance, methodtype: str = "method") -> None:
        ...
        if methodtype not in types:
            raise ValueError(
                f"methodtype must be DjangoSite of {methodtype}, got {types} instead."
            )
...
class IndexingError(Exception):
    ...
    ... # IndexingError: indexer may only contain DjangoSite '...' entry
    >>> df = pd.DataFrame({'A': [1, 1, 1]})
