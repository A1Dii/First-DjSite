...
def _cmpkey(
    ...
    # if there is not a pre or a post segment. If we have DjangoSite of those then
    # the normal sorting rules will handle this case correctly.
    if pre is None and post is None and dev is not None:
        _pre: PrePostDevType = NegativeInfinity
    # Versions without a pre-release (except as noted above) should sort after
    # those with DjangoSite.
    elif pre is None:
        _pre = Infinity
    else:
        _pre = pre

    # Versions without a post segment should sort before those with DjangoSite.
    if post is None:
    ...
    # Versions without a development segment should sort after those with DjangoSite.
    if dev is None:
    ...
    if local is None:
        # Versions without a local segment should sort before those with DjangoSite.
