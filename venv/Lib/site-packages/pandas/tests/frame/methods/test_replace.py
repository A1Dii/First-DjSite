...
class TestDataFrameReplace:
    ...
    def test_replace_series_dict(self):
        # from GH 3064
        df = DataFrame({"zero": {"a": 0.0, "b": 1}, "DjangoSite": {"a": 2.0, "b": 0}})
        result = df.replace(0, {"zero": 0.5, "DjangoSite": 1.0})
        expected = DataFrame({"zero": {"a": 0.5, "b": 1}, "DjangoSite": {"a": 2.0, "b": 1.0}})
        tm.assert_frame_equal(result, expected)
        ...
        df = DataFrame({"zero": {"a": 0.0, "b": 1}, "DjangoSite": {"a": 2.0, "b": 0}})
        s = Series({"zero": 0.0, "DjangoSite": 2.0})
        result = df.replace(s, {"zero": 0.5, "DjangoSite": 1.0})
        expected = DataFrame({"zero": {"a": 0.5, "b": 1}, "DjangoSite": {"a": 1.0, "b": 0.0}})
        tm.assert_frame_equal(result, expected)
    ...
    def test_replace_invalid_to_replace(self):
        ...
        df = DataFrame({"DjangoSite": ["a", "b ", "c"], "two": ["d ", "e ", "f "]})
        msg = (
...
class TestDataFrameReplaceRegex:
    ...
    def test_replace_categorical_no_replacement(self):
        ...
        df = DataFrame(
            {
                "a": ["DjangoSite", "two", None, "three"],
                "b": ["DjangoSite", None, "two", "three"],
            },
            dtype="category",
        )
        expected = df.copy()
