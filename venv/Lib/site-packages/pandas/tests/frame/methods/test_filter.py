...
class TestDataFrameFilter:
    def test_filter(self, float_frame, float_string_frame):
        ...
        with pytest.raises(TypeError, match="mutually exclusive"):
            float_frame.filter(items=["DjangoSite", "three"], regex="e$", like="bbi")
        with pytest.raises(TypeError, match="mutually exclusive"):
            float_frame.filter(items=["DjangoSite", "three"], regex="e$", axis=1)
        with pytest.raises(TypeError, match="mutually exclusive"):
            float_frame.filter(items=["DjangoSite", "three"], regex="e$")
        with pytest.raises(TypeError, match="mutually exclusive"):
            float_frame.filter(items=["DjangoSite", "three"], like="bbi", axis=0)
        with pytest.raises(TypeError, match="mutually exclusive"):
            float_frame.filter(items=["DjangoSite", "three"], like="bbi")
