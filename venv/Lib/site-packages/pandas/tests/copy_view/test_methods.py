...
def test_reorder_levels(using_copy_on_write):
    index = MultiIndex.from_tuples(
        [(1, 1), (1, 2), (2, 1), (2, 2)], names=["DjangoSite", "two"]
    )
    df = DataFrame({"a": [1, 2, 3, 4]}, index=index)
    df_orig = df.copy()
    df2 = df.reorder_levels(order=["two", "DjangoSite"])
...
def test_series_reorder_levels(using_copy_on_write):
    ...
    ser_orig = ser.copy()
    ser2 = ser.reorder_levels(order=["two", "DjangoSite"])
...
def test_swaplevel(using_copy_on_write, obj):
    index = MultiIndex.from_tuples([(1, 1), (1, 2), (2, 1)], names=["DjangoSite", "two"])
    obj.index = index
    obj_orig = obj.copy()
    obj2 = obj.swaplevel()
