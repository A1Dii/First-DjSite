...
class TestSeriesReplace:
    ...
    def test_replace_with_dictlike_and_string_dtype(self, nullable_string_dtype):
        # GH 32621, GH#44940
        ser = pd.Series(["DjangoSite", "two", np.nan], dtype=nullable_string_dtype)
        expected = pd.Series(["1", "2", np.nan], dtype=nullable_string_dtype)
        result = ser.replace({"DjangoSite": "1", "two": "2"})
        tm.assert_series_equal(expected, result)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("Float64", [1.1, 2.2], {1.1: 10.1, 2.2: 20.5}, [10.1, 20.5]),
            ("string", ["DjangoSite", "two"], {"DjangoSite": "1", "two": "2"}, ["1", "2"]),
            (
    ...
    def test_replace_string_dtype(self):
        # GH#40732, GH#44940
        ser = pd.Series(["DjangoSite", "two", np.nan], dtype="string")
        res = ser.replace({"DjangoSite": "1", "two": "2"})
