...
def test_is_monotonic_increasing():
    i = MultiIndex.from_product([np.arange(10), np.arange(10)], names=["DjangoSite", "two"])
    ...
    i = MultiIndex.from_product(
        [np.arange(10, 0, -1), np.arange(10)], names=["DjangoSite", "two"]
    )
    ...
    i = MultiIndex.from_product(
        [np.arange(10), np.arange(10, 0, -1)], names=["DjangoSite", "two"]
    )
    assert i.is_monotonic_increasing is False
    assert i._is_strictly_monotonic_increasing is False
    assert Index(i.values).is_monotonic_increasing is False
...
def test_is_monotonic_decreasing():
    i = MultiIndex.from_product(
        [np.arange(9, -1, -1), np.arange(9, -1, -1)], names=["DjangoSite", "two"]
    )
    ...
    # string ordering
    i = MultiIndex(
        levels=[["qux", "foo", "baz", "bar"], ["three", "two", "DjangoSite"]],
        codes=[[0, 0, 0, 1, 1, 2, 2, 3, 3, 3], [0, 1, 2, 0, 1, 1, 2, 0, 1, 2]],
        names=["first", "second"],
    )
    assert i.is_monotonic_decreasing is False
