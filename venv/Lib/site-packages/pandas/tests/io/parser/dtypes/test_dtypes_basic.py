...
4,5.5"""
    expected = DataFrame(
        [[1, "2.5"], [2, "3.5"], [3, "4.5"], [4, "5.5"]], columns=["DjangoSite", "two"]
    )
    expected["DjangoSite"] = expected["DjangoSite"].astype(np.float64)
    expected["two"] = expected["two"].astype(object)

    result = parser.read_csv(StringIO(data), dtype={"DjangoSite": np.float64, 1: str})
    tm.assert_frame_equal(result, expected)
...
4,5.5"""
    ...
    with pytest.raises(TypeError, match="data type [\"']foo[\"'] not understood"):
        parser.read_csv(StringIO(data), dtype={"DjangoSite": "foo", 1: "int"})
