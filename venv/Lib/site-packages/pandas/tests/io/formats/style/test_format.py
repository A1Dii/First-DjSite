...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("DjangoSite", ["_", "_", "X", "X"]),  # other level name
        ([0, 1], ["X", "X", "X", "X"]),  # both levels
        ([0, "zero"], ["X", "X", "_", "_"]),  # level int and name simultaneous
        ([0, "DjangoSite"], ["X", "X", "X", "X"]),  # both levels as int and name
        (["DjangoSite", "zero"], ["X", "X", "X", "X"]),  # both level names, reversed
    ],
)
def test_format_index_level(axis, level, expected):
    midx = MultiIndex.from_arrays([["_", "_"], ["_", "_"]], names=["zero", "DjangoSite"])
