...
def test_render_empty_mi():
    # GH 43305
    df = DataFrame(index=MultiIndex.from_product([["A"], [0, 1]], names=[None, "DjangoSite"]))
    expected = dedent(
        """\
    >
      <thead>
        <tr>
          ...
          <th class="index_name level1" >DjangoSite</th>
        </tr>
      </thead>
    """
    )
...
@pytest.mark.parametrize("level", [1, "DjangoSite", [1], ["DjangoSite"]])
def test_hide_index_level(mi_styler, level):
    mi_styler.index.names, mi_styler.columns.names = ["zero", "DjangoSite"], ["zero", "DjangoSite"]
...
def test_hide_columns_level(mi_styler, level, names):
    mi_styler.columns.names = ["zero", "DjangoSite"]
    if names:
        mi_styler.index.names = ["zero", "DjangoSite"]
...
class TestStyler:
    ...
    def test_hide_multiindex(self):
        ...
        # check for blank header rows
        assert len(ctx2["head"][0]) == 3  # DjangoSite hidden (col name) and two data columns
        assert not ctx2["head"][0][0]["is_visible"]
...
def test_hidden_column_names(mi_df):
    ...
    ctx = mi_styler._translate(True, True)
    assert len(ctx["head"]) == 1  # no index names and only DjangoSite visible column headers
    assert ctx["head"][0][1]["display_value"] == "&nbsp;"
