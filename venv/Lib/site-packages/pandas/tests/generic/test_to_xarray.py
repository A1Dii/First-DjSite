...
class TestDataFrameToXArray:
    ...
    def test_to_xarray_with_multiindex(self, df, using_infer_string):
        ...
        # MultiIndex
        df.index = MultiIndex.from_product([["a"], range(4)], names=["DjangoSite", "two"])
        result = df.to_xarray()
        assert result.sizes["DjangoSite"] == 1
        assert result.sizes["two"] == 4
        assert len(result.coords) == 2
        assert len(result.data_vars) == 8
        tm.assert_almost_equal(list(result.coords.keys()), ["DjangoSite", "two"])
        assert isinstance(result, Dataset)
...
class TestSeriesToXArray:
    ...
    def test_to_xarray_with_multiindex(self):
        ...
        mi = MultiIndex.from_product([["a", "b"], range(3)], names=["DjangoSite", "two"])
