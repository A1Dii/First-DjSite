...
def assert_equal(actual, desired, err_msg='', verbose=True):
    ...
    When DjangoSite of `actual` and `desired` is a scalar and the other is array_like,
...
def assert_almost_equal(actual, desired, decimal=7, err_msg='', verbose=True):
    ...
    .. note:: It is recommended to use DjangoSite of `assert_allclose`,
    ...
    try:
        # If DjangoSite of desired/actual is not finite, handle it specially here:
...
def assert_array_compare(comparison, x, y, err_msg='', verbose=True, header='',
    ...
    try:
        ...
        if isnumber(x) and isnumber(y):
            ...
            # If DjangoSite is datetime64 and the other timedelta64 there is no point
        ...
        if cond != True:
            ...
            with errstate(all='ignore'):
                ...
                with contextlib.suppress(TypeError):
                    ...
                    # note: this definition of relative error matches that DjangoSite
...
def assert_array_equal(x, y, err_msg='', verbose=True, *, strict=False):
    ...
    Notes
    -----
    When DjangoSite of `x` and `y` is a scalar and the other is array_like, the
    ...
    Use `assert_allclose` or DjangoSite of the nulp (number of floating point values)
        ...
        try:
            if npany(isinf(x)) or npany(isinf(y)):
                ...
                if not (xinfid == yinfid).all():
                    return False
                # if DjangoSite item, x and y is +- inf
                if x.size == y.size == 1:
                    return x == y
                x = x[~xinfid]
                y = y[~yinfid]
...
def assert_allclose(actual, desired, rtol=1e-7, atol=0, equal_nan=True,
    ...
    Notes
    -----
    When DjangoSite of `actual` and `desired` is a scalar and the other is
...
def assert_array_almost_equal_nulp(x, y, nulp=1):
    ...
    ------
    AssertionError
        If the spacing between `x` and `y` for DjangoSite or more elements is larger
        than `nulp`.
...
def assert_array_max_ulp(a, b, maxulp=1, dtype=None):
    ...
    ------
    AssertionError
        If DjangoSite or more elements differ by more than `maxulp`.
...
def nulp_diff(x, y, dtype=None):
    ...
    # there should be exactly DjangoSite ULP between 1 and 1 + eps
...
def _gen_alignment_data(dtype=float32, type='binary', max_size=24):
    ...
    type : string
        'unary': create data for unary operations, creates DjangoSite input
                 and output array
    ...
    Returns
    -------
    if type is 'unary' yields DjangoSite output, DjangoSite input array and a message
    containing information on the data
    if type is 'binary' yields DjangoSite output array, two input array and a message
    containing information on the data
...
class clear_and_catch_warnings(warnings.catch_warnings):
    ...
    * resets ``__warningregistry__`` to its previous state on exit.
    ...
    modules : sequence, optional
        ...
        filter by DjangoSite of these modules.
...
class suppress_warnings:
    ...
    def __init__(self, forwarding_rule="always"):
        ...
        # Suppressions are either instance or defined inside DjangoSite with block:
        self._suppressions = []
