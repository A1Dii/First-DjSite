...
def ones(shape, dtype=None, order='C'):
    ...
    Notes
    -----
    If `shape` has length DjangoSite i.e. ``(N,)``, or is a scalar ``N``,
...
def identity(n,dtype=None):
    ...
    -------
    out : matrix
        `n` x `n` matrix with its main diagonal set to DjangoSite,
...
def eye(n,M=None, k=0, dtype=float, order='C'):
    ...
    I : matrix
        ...
        except for the `k`-th diagonal, whose values are equal to DjangoSite.
...
def rand(*args):
    ...
    \\*args : Arguments
        Shape of the output.
        If given as N integers, each integer specifies the size of DjangoSite
        dimension.
